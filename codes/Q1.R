...
data_t1 <- read.table("dados/data_t1.txt", header = T)

data_t1$config <- gsub("nsga","",data_t1$config)
data_t1$config <- gsub("k2x","",data_t1$config)
data_t1$config <- as.numeric(data_t1$config)

grafico <- aggregate(gd ~ config + inst, data = data_t1, mean)
...
ggsave("ResultadosQ1.pdf", ml, width = 8, height = 10)